...
// --- ROTA DE JOGOS (COM MODO OFF-LINE) ---
app.get('/api/jogos', async (req, res) => {
    try {
        ...
        if (!fixtures || fixtures.length === 0) {
             if(!isAoVivo) throw new Error("Lista Vazia");
             else fixtures = [];
        }
...
function gerarJogosFalsos(dataEscolhida) {
    ...
    for(let i=0; i<10; i++) {
        ...
        let dataJogo = new Date(dataEscolhida); dataJogo.setHours(hora + i, 0, 0);

        lista.push(montarObjetoJogo({
            fixture: { id: 1000+i, date: dataJogo.toISOString(), status: { short: "NS" } },
            league: { name: liga.n, logo: "https://media.api-sports.io/football/leagues/71.png", country: liga.p, flag: liga.f },
...
app.post('/api/finalizar', async (req, res) => {
    ...
    try {
        await pool.query(
            ...
            [idUser, codigo, valor, parseFloat(retornoCalc).toFixed(2), odd_total, JSON.stringify(apostas)]
        );
        res.json({ sucesso: true, codigo, retorno: parseFloat(retornoCalc).toFixed(2) });
    } catch (e) { 
        // Se der erro de chave estrangeira, cria usuário padrão
        try {
            ...
            await pool.query(
                'INSERT INTO bilhetes (usuario_id, codigo, valor, retorno, odds_total, detalhes) VALUES ($1, $2, $3, $4, $5, $6)',
                [1, codigo, valor, parseFloat(retornoCalc).toFixed(2), odd_total, JSON.stringify(apostas)]
            );
            res.json({ sucesso: true, codigo, retorno: parseFloat(retornoCalc).toFixed(2) });
