...
const CONFIG = {
    // Tenta pegar a chave do Railway, senão usa o texto (mas prefira usar no Railway)
    API_KEY: process.env.API_FOOTBALL_KEY || "SUA_CHAVE_AQUI", 
    LUCRO_CASA: 0.94, 
    TEMPO_CACHE_MINUTOS: 60, // Atualiza a cada 1 hora para economizar
    MIN_VALOR: 2.00,
    MAX_VALOR: 1000.00,
    MAX_PREMIO: 10000.00
};

// LISTA DE FORTES V75 (Com Bragantino, Fortaleza, etc)
const TIMES_FORTES = [
...
async function initDb() {
    try {
        ...
        console.log("✅ Servidor V75 (Proteção de Odds + Economia) Online!");
...
app.get('/api/jogos', async (req, res) => {
    ...
    try {
        ...
        console.log(`🌍 V75: Buscando Jogos e Odds (Econômico)...`);
        ...
        // 2. Odds (Apenas página 1 para economizar)
        let mapaOdds = {};
        try {
        ...
        } catch (e) { console.log("⚠️ API Odds falhou ou limitou. Usando simulador."); }

        let jogosFinais = formatarV75(listaBruta, mapaOdds, dataHoje);
        ...
        // Se der erro (ex: sem internet), tenta usar cache antigo
...
// --- ROTA FINALIZAR (COM VERIFICAÇÃO DE MUDANÇA DE ODDS) ---
        ...
        if(cache.rows.length > 0 && !forcar) {
            ...
            for(let aposta of apostas) {
                ...
                if(jogoAtual) {
                    ...
                    // Se a diferença for maior que 0.05, avisa o cliente
                    if(oddAtual && Math.abs(parseFloat(oddAtual) - parseFloat(aposta.odd)) > 0.05) {
            ...
            if(mudancas.length > 0) {
                return res.json({ sucesso: false, aviso_mudanca: true, mudancas: mudancas });
            }
        }
...
function formatarV75(listaJogos, mapaOdds, dataFiltro) {
...
app.listen(process.env.PORT || 3000, () => console.log("🔥 Server V75 On!"));
