...
const CONFIG = {
    ...
    LUCRO_CASA: 0.90, 
    // CACHE DE 2 MINUTOS: Para o placar atualizar rápido e o status mudar logo
...
async function initDb() {
    try {
        ...
        await pool.query("DELETE FROM jogos_cache"); // Limpa memória ao reiniciar
        console.log("✅ Servidor V61 (Placar + Data Fix) Online!");
...
app.get('/api/jogos', async (req, res) => {
    ...
    try {
        ...
        console.log(`🌍 V61: Atualizando jogos de ${dataHoje}...`);
        const headers = { 'x-apisports-key': CONFIG.API_KEY };

        // 1. Busca Jogos (Com Timezone BR)
        ...
        // 2. Busca Odds (Página 1 e 2 para garantir estaduais)
        let mapaOdds = {};
        try {
            // Página 1
            const r1 = await axios.get(`https://v3.football.api-sports.io/odds?date=${dataHoje}&bookmaker=6&timezone=America/Sao_Paulo&page=1`, { headers });
            if(r1.data.response) r1.data.response.forEach(o => { mapaOdds[o.fixture.id] = o.bookmakers[0].bets; });
            
            // Página 2 (Tenta buscar o resto)
            if(r1.data.paging && r1.data.paging.total > 1) {
                const r2 = await axios.get(`https://v3.football.api-sports.io/odds?date=${dataHoje}&bookmaker=6&timezone=America/Sao_Paulo&page=2`, { headers });
                if(r2.data.response) r2.data.response.forEach(o => { mapaOdds[o.fixture.id] = o.bookmakers[0].bets; });
            }
        } catch (e) { console.log("⚠️ API Odds limitou ou falhou."); }

        let jogosFinais = formatarV61(listaBruta, mapaOdds, dataHoje);
...
function formatarV61(listaJogos, mapaOdds, dataFiltro) {
    return listaJogos.map(j => {
        try {
            // --- FILTRO DE DATA RIGOROSO ---
            // A API retorna "2026-01-18T00:30:00-03:00". Pegamos só a data local.
            const dataLocal = j.fixture.date.substring(0, 10); 
            if (dataLocal !== dataFiltro) return null; // Se não é hoje, some.
            ...
            // Jogo Ao Vivo (Flamengo agora)
            else if (['1H', '2H', 'HT', 'ET', 'P', 'BT', 'INT', 'LIVE'].includes(st)) {
                statusFinal = "LIVE";
                placar = { home: j.goals.home ?? 0, away: j.goals.away ?? 0 };
            }

            // ODDS (Só existem se o jogo for VS)
            let oddsFinais = null;
            ...
            if (statusFinal === "VS") {
                ...
                mercadosCalculados = gerarMercadosCompletosV61(oddsBase);
            }
            ...
            return {
                ...
                data: j.fixture.date,
                status: statusFinal, // 'VS', 'FT' ou 'LIVE'
                placar: placar,
                odds: oddsFinais,
...
function gerarMercadosCompletosV61(base) {
    ...
    let bttsYes = 1.95; 
    if(E < 3.2) bttsYes = 1.75; 
    if(C < 1.4 || F < 1.4) bttsYes = 2.10;
...
app.listen(process.env.PORT || 3000, () => console.log("🔥 Server V61 (Full Clean) On!"));
