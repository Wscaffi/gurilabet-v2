...
// =====================================================
// CONFIGURAÇÕES V90 - MODO ECONÔMICO (ANTI-BLOQUEIO)
// =====================================================
const CONFIG = {
    API_KEY: process.env.API_FOOTBALL_KEY || "SUA_CHAVE_AQUI", 
    LUCRO_CASA: 0.98, 
    
    // Cache de 30 minutos. (Ideal para plano grátis)
    // O site atualiza as odds a cada 30 min, ou se você reiniciar.
    ...
    // Reduzi para 6 páginas para garantir que dê tempo de buscar tudo em 1 minuto
    // 6 páginas x 10 jogos = 60 jogos com odds completas (os principais)
    MAX_PAGINAS_ODDS: 6 
};

// LISTA DE TIMES FORTES (Fallback de segurança)
const TIMES_FORTES = [
...
async function initDb() {
    try {
        ...
        console.log("✅ Servidor V90 (Modo Econômico Seguro) Online!");
    } catch (e) { console.error("Erro Banco:", e.message); }
}
initDb();

// --- ROTA DE JOGOS (COM FREIO ABS) ---
app.get('/api/jogos', async (req, res) => {
    ...
    try {
        ...
        if (cache.rows.length > 0) {
            ...
            // Se o cache for recente, usa ele e NÃO GASTA REQUISIÇÃO
        ...
        console.log(`🌍 V90: Atualizando Odds (Lentamente para não bloquear)...`);
        const headers = { 'x-apisports-key': CONFIG.API_KEY };
        
        // 1. Pega Lista de Jogos (Gasta 1 requisição)
        ...
        try {
            ...
            do {
                console.log(`   ↳ Baixando odds página ${page}...`);
                
                // Gasta 1 requisição por página
                ...
                // Espera 7 segundos entre cada busca.
                // Isso garante aprox. 8 requisições por minuto (abaixo do limite de 10).
                if(page <= totalPages && page <= CONFIG.MAX_PAGINAS_ODDS) {
                    await new Promise(resolve => setTimeout(resolve, 7000));
                }

            } while (page <= totalPages && page <= CONFIG.MAX_PAGINAS_ODDS);
        } catch (e) { console.log("⚠️ API Odds limitou. Continuando com o que temos."); }

        let jogosFinais = formatarV90(listaBruta, mapaOdds, dataHoje);
...
// --- LÓGICA V90 ---
function formatarV90(listaJogos, mapaOdds, dataFiltro) {
        ...
        try {
            ...
            if (statusFinal === "VS") {
                ...
                if (!oddsBase) {
                    ...
                    // Se for time grande e a API falhou, gera. Se for pequeno, bloqueia.
                    if (ehTimeGrande) { mercadosCalculados = gerarListaSintetica(oddsBase); } else { mercadosCalculados = []; }
                } else {
                    oddsFinais = { 
                        casa: (oddsBase.casa * CONFIG.LUCRO_CASA).toFixed(2), 
                        empate: (oddsBase.empate * CONFIG.LUCRO_CASA).toFixed(2), 
...
function gerarListaHibrida(base, betsApi, ehGrande) {
    ...
    const m5 = getM(5); const golsItems = [];
    [0.5, 1.5, 2.5, 3.5, 4.5].forEach(g => { let found = false; if(m5) { const o = findOdd(m5, `Over ${g}`), u = findOdd(m5, `Under ${g}`); if(o && u) { golsItems.push({nome: `Mais ${g}`, odd: fx(o)}); golsItems.push({nome: `Menos ${g}`, odd: fx(u)}); found = true; } } if(!found && ehGrande && g === 2.5) { golsItems.push({nome: `Mais 2.5`, odd: fx(1.90)}); golsItems.push({nome: `Menos 2.5`, odd: fx(1.85)}); } });
    if(golsItems.length) lista.push({ grupo: "Total de Gols", itens: golsItems });
    const m8 = getM(8); if(m8) { const s = findOdd(m8, 'Yes'), n = findOdd(m8, 'No'); if(s && n) lista.push({ grupo: "Ambas as equipes marcam", itens: [{nome: "Sim", odd: fx(s)}, {nome: "Não", odd: fx(n)}] }); } else if(ehGrande) { lista.push({ grupo: "Ambas as equipes marcam", itens: [{nome: "Sim", odd: fx(1.90)}, {nome: "Não", odd: fx(1.80)}] }); }
    const m12 = getM(12); if(m12) { const ce = findOdd(m12, 'Home/Draw'), cf = findOdd(m12, 'Home/Away'), ef = findOdd(m12, 'Draw/Away'); if(ce && cf && ef) lista.push({ grupo: "Chance Dupla", itens: [{nome: "Casa/Empate", odd: fx(ce)}, {nome: "Casa/Fora", odd: fx(cf)}, {nome: "Empate/Fora", odd: fx(ef)}] }); } else if(ehGrande) { const dcC = 1 / (1/base.casa + 1/base.empate); const dcF = 1 / (1/base.casa + 1/base.fora); const dcE = 1 / (1/base.empate + 1/base.fora); lista.push({ grupo: "Chance Dupla", itens: [{nome: "Casa/Empate", odd: fx(dcC)}, {nome: "Casa/Fora", odd: fx(dcF)}, {nome: "Empate/Fora", odd: fx(dcE)}] }); }
...
app.listen(process.env.PORT || 3000, () => console.log("🔥 Server V90 (Economico) On!"));
